-- ============================================================
-- xxl-job 2.5.0 基础设施（xxl_job 库 + 8 张表 + 账号/锁）
--
-- 职责：admin（调度中心）运行所依赖的表结构。
--       来源官方 doc/db/tables_xxl_job.sql，仅做 IF NOT EXISTS 幂等化；
--       并删去官方示例执行器种子数据（xxl-job-executor-sample，本系统不使用）。
-- 说明：xxl-job admin 无自动建表机制，缺表会反复重启，故本文件必须存在。
-- 业务调度规则（执行器组 + 任务预置）不在此文件 —— 见 xxl-job-init.sql。
-- 顺序：initdb 按文件名字母序执行，本文件先于 xxl-job-init.sql。
-- ============================================================
SET NAMES utf8mb4;
...
-- ---------- 种子数据 ----------
-- 默认登录账号：admin / 123456（密码为 MD5，admin UI 登录用）
