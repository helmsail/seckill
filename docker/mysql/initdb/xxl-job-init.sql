-- xxl-job 2.5.0 表结构（docker mysql 首次初始化时自动执行；来源：官方 doc/db/tables_xxl_job.sql 结构）
-- 默认登录账号：admin / 123456
-- 说明：initdb 脚本默认执行在 MYSQL_DATABASE（seckill）库，此处显式建库并切换
...
SELECT 'xxl-job-executor-sample', '示例执行器', 0, NULL, NOW()
WHERE NOT EXISTS (SELECT 1 FROM `xxl_job_group` WHERE `app_name` = 'xxl-job-executor-sample');
