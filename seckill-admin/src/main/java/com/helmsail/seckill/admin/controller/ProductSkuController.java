...
public class ProductSkuController {
    ...
    /** 写：deductStock/addStock 非幂等，方法级禁重试；读走引用默认 */
    @DubboReference(methods = {
            @Method(name = "deductStock", retries = 0),
            @Method(name = "addStock", retries = 0)
    })
        ...
        try {
            ...
            for (AddProductSkuRequest.Item item : deductedItems) {
                try {
                    ...
                    log.error("补偿归还库存失败，需人工核对: skuNo={}, activityNo={}",
                            item.getSkuNo(), request.getActivityNo(), ex);
                }
            }
            throw e;
        }
     ...
     * 批量删除（物理删除，仅待开始状态）
     *
     * 编排：秒杀域删除并取回需归还清单 → 归还主域（失败仅记日志，需人工核对）
     */
    @DeleteMapping
    public Result<Void> batchRemove(@Valid @RequestBody RemoveProductSkuRequest request) {
        ...
        for (StockRestoreItem item : restoreItems) {
            try {
                ...
                log.error("归还主域库存失败，需人工核对: skuNo={}, stock={}, activityNo={}",
                        item.getSkuNo(), item.getStockToRestore(), request.getActivityNo(), e);
            }
        }
        return Result.success();
    }
